...
const getChirpsForGivenAuthorAsc = `-- name: GetChirpsForGivenAuthorAsc :many
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE user_id = $1
ORDER BY created_at ASC
...
func (q *Queries) GetChirpsForGivenAuthorAsc(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsForGivenAuthorAsc, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getChirpsForGivenAuthorDesc = `-- name: GetChirpsForGivenAuthorDesc :many
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE user_id = $1
ORDER BY created_at DESC
...
func (q *Queries) GetChirpsForGivenAuthorDesc(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsForGivenAuthorDesc, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
